...
export class DojoColumnDialog extends HTMLElement {
    ...
    openRename(columnId, currentName) {
        this._mode = 'rename';
    ...
    openDelete(columnId, columnName, otherColumns) {
        this._mode = 'delete';
        this._columnId = columnId;
    ...
    _buildContent() {
        ...
        if (this._mode === 'create')
            this._buildCreateForm(dialog);
        if (this._mode === 'rename')
            this._buildRenameForm(dialog);
        if (this._mode === 'delete')
            this._buildDeleteConfirm(dialog);
    }
    // ── Formulario: Crear columna ─────────────────────────────────────────────
    _buildCreateForm(container) {
    ...
    _open() {
        ...
        if (backdrop) {
            ...
            backdrop.setAttribute('aria-label', this._mode === 'create' ? 'Crear columna' :
