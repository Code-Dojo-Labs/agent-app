...
export class DojoCommandPalette extends HTMLElement {
    static TAG = 'dojo-command-palette';
    _shadow;
    _tasks = [];
    _results = [];
    _selectedIndex = -1;
    ...
    _onDocKeydown = (e) => {
        ...
        if ((e.metaKey || e.ctrlKey) && e.key === 'k') {
            ...
            if (openModal && openModal !== this._shadow.querySelector('.palette'))
                return;
            e.preventDefault();
    ...
    async show() {
        this._previousFocus = (this._shadow.activeElement ?? document.activeElement);
    ...
    _renderResults() {
        ...
        this._createOption.setAttribute('aria-selected', this._selectedIndex === this._results.length && hasQuery ? 'true' : 'false');
        this._updateActiveDescendant();
    }
