...
export class DojoDeleteConfirmDialog extends HTMLElement {
    static TAG = 'dojo-delete-confirm-dialog';
    _shadow;
    _taskId = '';
    ...
    // ── API pública ───────────────────────────────────────────────────────────
    show(taskId, taskTitle, triggerEl) {
        this._taskId = taskId;
        this._triggerEl = triggerEl ?? null;
        const titleEl = this._shadow.querySelector('.dialog-task-title');
        if (titleEl)
        ...
        requestAnimationFrame(() => {
            this._shadow.querySelector('.cancel-btn')?.focus();
        });
    }
    hide() {
    ...
    _getFocusable() {
        ...
        if (!dialog)
            return [];
        return Array.from(dialog.querySelectorAll('button:not([disabled])')).filter(el => el.offsetParent !== null);
    }
    // ── Render ────────────────────────────────────────────────────────────────
    _render() {
