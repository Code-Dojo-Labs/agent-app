...
export class DojoApp extends HTMLElement {
    ...
    _onImportKeydown = (e) => {
        if (!this.hasAttribute('import-confirm'))
            return;
        if (e.key === 'Escape') {
            e.preventDefault();
        ...
        if (e.key === 'Tab') {
            const focusable = Array.from(this._shadow.querySelectorAll('.import-dialog button:not([disabled])')).filter(el => el.offsetParent !== null);
    ...
    `;
        ...
        this._shadow.addEventListener('dojo:board-selected', (e) => {
        ...
        this._shadow.addEventListener('dojo:palette-select-task', (e) => {
            const { taskId } = e.detail;
            board.openTaskById?.(taskId);
        });
        // Crear tarea desde la paleta
        this._shadow.addEventListener('dojo:palette-create-task', (e) => {
            const { title } = e.detail;
            board.openCreateTaskWithTitle?.(title);
        });
        ...
        this._shadow.addEventListener('dojo:label-updated', (e) => {
            const { label } = e.detail;
            board.refreshLabel?.(label);
        });
        // Cuando se elimina una etiqueta, propagar al tablero para eliminar sus chips (US-12)
        this._shadow.addEventListener('dojo:label-deleted', (e) => {
    ...
    _navigateToBoard(boardId) {
        this._activeBoardId = boardId;
        this.setAttribute('view', 'board');
        const selector = this._shadow.querySelector('dojo-board-selector');
        const board = this._shadow.querySelector('dojo-kanban-board');
        const palette = this._shadow.querySelector('dojo-command-palette');
        if (selector)
    ...
    _showImportConfirm() {
        ...
        const dialog = this._shadow.querySelector('.import-dialog');
        dialog?.setAttribute('aria-hidden', 'false');
        document.removeEventListener('keydown', this._onImportKeydown);
        document.addEventListener('keydown', this._onImportKeydown);
        requestAnimationFrame(() => {
            this._shadow.querySelector('.import-cancel-btn')?.focus();
        });
    }
    _hideImportConfirm() {
