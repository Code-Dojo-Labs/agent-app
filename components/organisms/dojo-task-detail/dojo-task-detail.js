...
export class DojoTaskDetail extends HTMLElement {
    static TAG = 'dojo-task-detail';
    _shadow;
    /** Snapshot local de la tarea en edición. */
    _task = null;
    _columns = [];
    _allLabels = [];
    _labelsPickerOpen = false;
    ...
    _onDocKeydown = (e) => {
        ...
        if (e.key === 'Tab') {
            ...
            if (!panel)
                return;
            const focusable = Array.from(panel.querySelectorAll('button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])')).filter(el => !el.hidden && el.offsetParent !== null);
    ...
    _openPanel() {
        ...
        requestAnimationFrame(() => {
            this._shadow.querySelector('.title-input')?.focus();
        });
    }
    _closePanel() {
    ...
    _buildTitleField(task) {
        ...
        input.addEventListener('blur', () => {
            ...
            if (trimmed !== titleSnapshot)
                this._save({ title: trimmed });
        });
        section.appendChild(lbl);
        section.appendChild(input);
        return section;
    }
    _buildStatusPriorityRow(task) {
        ...
        priSelect.addEventListener('change', () => {
            this._save({ priority: priSelect.value });
        });
    ...
    _buildDescriptionField(task) {
        ...
        textarea.addEventListener('blur', () => {
            ...
            if (trimmed !== descSnapshot.trim())
                this._save({ description: trimmed });
        });
        // Preview
    ...
    _buildLabelsField(task) {
        ...
        picker.addEventListener('keydown', (ev) => {
            if (ev.key === 'Escape') {
                ...
                if (showCreateForm) {
                    ...
                    renderPicker();
                    requestAnimationFrame(() => picker.querySelector('.labels-search')?.focus());
                }
                else {
                    this._labelsPickerOpen = false;
                    picker.classList.remove('open');
                    chips.querySelector('.add-label-btn')?.focus();
                }
                return;
            }
            if (ev.key === 'ArrowDown' || ev.key === 'ArrowUp') {
                const focusable = Array.from(picker.querySelectorAll('input[type="checkbox"], .label-create-option, .color-swatch, .label-btn, .label-btn-primary, input[type="color"]'));
                if (focusable.length < 2)
                    return;
                const current = focusable.indexOf(this._shadow.activeElement);
                if (ev.key === 'ArrowDown') {
                    ev.preventDefault();
        ...
        const renderChips = () => {
            ...
            currentIds.forEach(labelId => {
                ...
                if (HEX_COLOR_RE.test(label.color)) {
                    try {
                        textColor = pickTextColor(label.color);
                    }
                    catch { /* sin cambios */ }
                }
                else {
        ...
        const buildCreateForm = (name) => {
            ...
            PRESET_COLORS.forEach(c => {
                ...
                swatch.addEventListener('click', () => {
                    ...
                    const ci = form.querySelector('.label-color-input');
                    if (ci)
                        ci.value = c;
                });
            ...
            const updateContrastWarning = (color) => {
            ...
            confirmBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    this._allLabels = [...this._allLabels, newLabel].sort((a, b) => a.name.localeCompare(b.name, 'es', { sensitivity: 'base' }));
                    const currentIds = this._task?.labelIds ?? [];
        ...
        const renderPicker = () => {
            ...
            searchInput.addEventListener('input', () => {
                ...
                if (showCreateForm) {
                    const hasExact = this._allLabels.some(l => l.name.toLowerCase() === trimmedNew.toLowerCase());
                    if (!trimmedNew || hasExact)
                        showCreateForm = false;
                }
                renderPicker();
                requestAnimationFrame(() => {
                    const s = picker.querySelector('.labels-search');
                    if (s) {
                        s.focus();
            ...
            if (trimmed) {
                const hasExact = this._allLabels.some(l => l.name.toLowerCase() === trimmed.toLowerCase());
                if (!hasExact) {
                    if (showCreateForm) {
                        ...
                        createOpt.addEventListener('keydown', (ev) => {
    ...
    _buildSubtasksField(task) {
        ...
        const addSubtask = () => {
            ...
            if (!text || !this._task)
                return;
            const newSub = { id: generateUUID(), text, completed: false };
            ...
            requestAnimationFrame(() => {
                section.querySelector('.subtask-add-input')?.focus();
            });
        };
    ...
    _buildSubtaskItem(sub, allSubtasks) {
        ...
        checkbox.addEventListener('change', () => {
            if (!this._task)
                return;
            const updated = (this._task.subtasks ?? []).map(s => s.id === sub.id ? { ...s, completed: checkbox.checked } : s);
            this._save({ subtasks: updated });
            // Reconstruir la sección para actualizar progreso
            const section = li.closest('.subtasks-section');
            if (section)
                this._rebuildSubtasksSection(section);
        });
    ...
    _rebuildSubtasksSection(oldSection) {
        if (!this._task)
            return;
    ...
    async _loadActivitySection(taskId, container) {
        try {
    ...
    _renderActivityEvents(events, container) {
        container.innerHTML = '';
        const heading = document.createElement('h3');
    ...
    _activityDescription(evt) {
        ...
        switch (evt.type) {
            ...
            case 'priority_change':
                return `Prioridad cambiada de "${this._priorityLabel(p.from)}" a "${this._priorityLabel(p.to)}"`;
            case 'label_added':
