...
export class DojoBoardSelector extends HTMLElement {
    static TAG = 'dojo-board-selector';
    _shadow;
    _boards = [];
    constructor() {
        super();
    ...
    _renderBoardGrid() {
        ...
        newCard.addEventListener('click', () => this._showCreateForm(grid));
    ...
    _showDeleteDialog(board) {
        if (this._boards.length <= 1) {
            // No permitir eliminar el último tablero
            const body = this._shadow.querySelector('.delete-dialog-body');
            if (body)
                body.textContent = 'No se puede eliminar el único tablero. Debe existir al menos un tablero.';
            const confirmBtn = this._shadow.querySelector('.delete-confirm-btn');
            if (confirmBtn)
        ...
        const body = this._shadow.querySelector('.delete-dialog-body');
        if (body)
            body.textContent = `Se eliminará el tablero "${board.name}" junto con todas sus columnas y tareas. Esta acción no se puede deshacer.`;
        const confirmBtn = this._shadow.querySelector('.delete-confirm-btn');
        if (confirmBtn) {
    ...
    _showDeleteDialogUI() {
        ...
        requestAnimationFrame(() => {
            this._shadow.querySelector('.delete-cancel-btn')?.focus();
        });
    }
    _hideDeleteDialog() {
    ...
    _onDeleteKeydown = (e) => {
        ...
        if (e.key === 'Tab') {
            const focusable = Array.from(this._shadow.querySelectorAll('.delete-dialog button:not([style*="display: none"])')).filter(el => el.offsetParent !== null);
