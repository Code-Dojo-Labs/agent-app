 ...
 */
export async function getActivitiesByTaskId(taskId) {
    const { store } = await getStore('activity');
    const index = store.index('by-taskId');
    const events = await idbRequest(index.getAll(taskId));
 ...
 */
export async function deleteActivitiesByTaskId(taskId) {
    const db = await openDatabase();
    const tx = db.transaction('activity', 'readwrite');
    const store = tx.objectStore('activity');
    const index = store.index('by-taskId');
    const keys = await idbRequest(index.getAllKeys(taskId));
