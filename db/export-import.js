...
export async function exportBoardData() {
    ...
    const storeNames = ['columns', 'tasks', 'labels'];
    ...
    const columns = await idbRequest(tx.objectStore('columns').getAll());
    const tasks = await idbRequest(tx.objectStore('tasks').getAll());
    const labels = await idbRequest(tx.objectStore('labels').getAll());
    const activity = hasActivity
        ? await idbRequest(tx.objectStore('activity').getAll())
        : [];
    const boards = hasBoards
        ? await idbRequest(tx.objectStore('boards').getAll())
        : [];
    const projects = hasProjects
        ? await idbRequest(tx.objectStore('projects').getAll())
        : [];
    return {
 ...
 */
export function downloadBoardExport(data) {
...
export async function importBoardData(data) {
    ...
    if (hasBoards) {
        ...
        if (data.boards && data.boards.length > 0) {
            ...
            for (const col of data.columns) {
                if (!col.boardId)
                    colStore.put({ ...col, boardId: defaultId });
            }
            for (const task of data.tasks) {
                if (!task.boardId)
                    taskStore.put({ ...task, boardId: defaultId });
            }
        }
    }
