...
export async function getAllPersons(): Promise<Person[]> {
  ...
  const persons = await idbRequest<Person[]>(store.getAll());
...
export async function getPersonById(id: string): Promise<Person | undefined> {
  const { store } = await getStore('persons');
  return idbRequest<Person | undefined>(store.get(id));
}
...
export async function getPersonsByIds(ids: string[]): Promise<Person[]> {
  ...
  for (const id of ids) {
    const person = await idbRequest<Person | undefined>(store.get(id));
    if (person) {
      persons.push(person);
    }
  }
 ...
 */
export async function searchPersonsByName(query: string): Promise<Person[]> {
  if (!query.trim()) return getAllPersons();
 ...
 */
export async function updatePerson(id: string, changes: UpdatePersonInput): Promise<Person> {
...
export async function deletePerson(id: string): Promise<void> {
  ...
  if (!person) return; // Silencioso si no existe
  ...
  try {
    ...
    const allTasks = await idbRequest<import('../types/models.js').Task[]>(taskStore.getAll());
